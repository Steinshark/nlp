...
parser.add_argument('--learning_rate', metavar='LR', type=float, default=0.0002, help='Learning rate for Adam')
...
def train(model_name="117M",models_dir="models",batch_size=1,noise=0.0,val_every=0,sample_every=50,save_every=50,optimizer="adam",lr=.00002,val_batch_size=2,
        top_k=40,top_p=0.0,sample_length=1023,memory_saving_gradients=False,twremat=False,twremat_memlimit="12G",run_name="run1",
        restore_from='latest',sample_num=1,dataset="data",encoding="utf-8",val_dataset="",val_batch_count=40,combine=50000):
    
    enc = encoder.get_encoder(model_name=model_name, models_dir=models_dir)
    hparams = model.default_hparams()
    ...
    with tf.Session() as sess:
        # Fully static shape required to make memory accounting in
        # twremat accurate.
        train_context = tf.placeholder(tf.int32, [batch_size, 1024])
        train_context_in = randomize(train_context, hparams, noise)
        train_output = model.model(hparams=hparams, X=train_context_in)
        train_loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=train_context[:, 1:], logits=train_output['logits'][:, :-1]))
        ...
        sample_context = tf.placeholder(tf.int32, [batch_size, None])
        tf_sample = sample.sample_sequence(
            hparams=hparams,
        ...
        if memory_saving_gradients:
            ...
            import tfremat
            opt_grads = tf.gradients(train_loss, train_vars)
            (train_loss, opt_grads) = tfremat.tf_remat((train_loss, opt_grads), memlimit=twremat_memlimit)
        else:
            opt_grads = tf.gradients(train_loss, train_vars)
        opt_grads = list(zip(opt_grads, train_vars))
        opt_apply = opt.apply_gradients(opt_grads)
        ...
        def generate_samples():
            ...
            while index < sample_num:
                out = sess.run(
                    tf_sample,
                    feed_dict={sample_context: batch_size * [context_tokens]})
